...
| **LLM integration** | LangGraph 5-node pipeline, 6 parallel personas + **portfolio-level 10-section comprehensive spec**, UUID-grounded citations, SSE streaming | Single-shot LLM prompt, no grounding |
...
### 9. Portfolio-Level Comprehensive Specification — 10 Sections, Full English Narrative

The **Portfolio Comprehensive Report** runs against the entire corpus with no program selection required. It generates a single unified document covering every program, all business rules, all data structures, and the complete migration strategy — written in full English narrative prose, not data dumps.

**Structure (10 sections, SSE-streamed):**

| # | Section | Type | What It Contains |
|---|---------|------|-----------------|
| 1 | Portfolio Overview | Static | Stats table, inter-program call graph (Mermaid), JCL batch pipeline (Mermaid), CICS transaction flow (Mermaid) |
| 2 | Program Inventory | Static | All 31 programs with LOC, paragraph count, rule count, risk count; shared copybook data model |
| 3 | Business Context & Domain Analysis | LLM | System overview, capability domain map, end-to-end workflow narratives, business rules in plain English, batch job business purposes, core entity model with ER diagram |
| 4 | Functional Specs — Online Programs | LLM | Per-program: business purpose (4+ sentences), input/output tables, step-by-step processing in English, every business rule explained as a policy statement, error scenarios |
| 5 | Functional Specs — Batch Programs & Data Architecture | LLM | Batch program specs with JCL context + restart/recovery; ER diagram; copybook-to-Java mapping; COMP-3 field inventory; 88-level condition → Java enum catalogue; PostgreSQL DDL |
| 6 | Technical Architecture & Java Scaffolding | LLM | Technology justification paragraphs; architecture Mermaid diagram; Maven module structure; JPA @Entity classes; Spring Data repos; @RestController skeletons; Spring Batch @Configuration classes |
| 7 | Migration Strategy & Execution Roadmap | LLM | Strangler Fig rationale; microservice decomposition; 6-phase narrative plan; Mermaid Gantt chart; data migration strategy; parallel-run reconciliation approach |
| 8 | Risk Analysis & Mitigation Plan | LLM | Each HIGH risk explained in full English with business impact; MEDIUM risks by category; risk heat map (Mermaid quadrantChart); risk-driven migration sequencing |
| 9 | Testing Strategy & Quality Assurance | LLM | Business rule unit test catalogue; integration test design; parallel-run reconciliation framework; batch regression tests; performance benchmarks |
| 10 | Complete Appendix — All Artifacts | Static | All 384 business rules, all 88-level conditions, all arithmetic specs, complete call graph, file I/O, JCL bindings, risk register — direct from DB |

**Key design decisions:**
- Sections 3–9 are generated by 7 parallel LLM calls (`concurrent.futures.ThreadPoolExecutor`) — sections stream in as they complete, not waiting for all to finish
- `SPEC_SYSTEM` prompt enforces a minimum 1,500-word narrative per section, with English paragraph prose required before any table or code block
- Static sections 1, 2, and 10 render instantly from the DB — no LLM tokens used
- Mermaid diagrams in sections 1 and 6–8 are generated from actual DB data (not templates)
- Inline progress tracker shows all 10 section names with live ✓ completion status — sticky at top of content panel, fades out on completion
- Output is fully rendered markdown: tables, code blocks, Mermaid diagrams, headings — not raw markdown text
- Scrollable without page-width overflow (CSS Grid `min-width: 0` fix + `overflow-x: hidden`)

### 10. Interactive Knowledge Graph — Click-to-Explain with AI

The vis.js knowledge graph renders the complete inter-program dependency topology. Clicking any node opens a panel with an **"Explain with AI" button** that fires a live LLM call — grounded in the artifact slice for that node — and returns a full specification document. This works for all three node types:

- **COBOL programs** (`POST /generate-spec`) — 6-persona spec with LangGraph grounding
- **Copybooks** (`POST /explain-copybook`) — 5-section full spec: business purpose, field-by-field explanation, data flow analysis, Java class skeleton, migration considerations
- **JCL batch jobs** (`POST /explain-jcl`) — 6-section full spec: business purpose, step-by-step logic, business rules in plain English, Mermaid data-flow diagram, Spring Batch @Configuration skeleton, migration risks

Previously, JCL nodes showed only raw metadata and copybooks showed a 3-sentence summary. All three node types now produce consistent full-depth specifications.
...
### 5. LangGraph Spec Generation — 6 Parallel Personas + Portfolio Report
...
The **"Generate Comprehensive Report"** button (no program selection required) triggers `POST /generate-spec/comprehensive` — a portfolio-wide, corpus-level specification document. 10 sections stream in via SSE as they complete. An inline progress checklist at the top of the content panel shows the status of all 10 sections in real time, fading out once generation is complete. Full details in the [Portfolio-Level Comprehensive Specification](#9-portfolio-level-comprehensive-specification--10-sections-full-english-narrative) section above.
...
| POST | `/generate-spec` | LangGraph spec generation (single program/paragraph) |
| POST | `/generate-spec/personas` | SSE-streamed 6-persona parallel spec |
| POST | `/generate-spec/comprehensive` | Portfolio-wide 10-section comprehensive spec (SSE-streamed) |
| POST | `/explain-copybook` | Full 5-section LLM spec for a COBOL copybook |
| POST | `/explain-jcl` | Full 6-section LLM spec for a JCL batch job |
...
| **3** | **Spec Generation** (grounded, demo-able) | 15% | LangGraph 5-node pipeline; 6 parallel personas; **portfolio-wide 10-section comprehensive spec**; grounding score per output; Markdown + PDF export | ✅ |
